...
def login():
    if request.method == 'POST':
        ...
        cur.execute("SELECT * FROM data_admin WHERE email=%s and password=%s", (email,password,))
        user = cur.fetchone()
        cur.close()
...
def index():
    ...
    cur.execute("SELECT DISTINCT Nama_Produk, Harga_Jual, sum(jumlah_produk_dibeli) FROM data_penjualan group by nama_produk;")
    produk = cur.fetchall()
    cur.close()
...
def simpanformmasterbarang():  
    ...
    #koneksi
    cur = mysql.connection.cursor()
    cur.execute("INSERT INTO data_produk VALUES (%s,%s,%s,%s)", (nama, harga,foto,deskripsi))
    mysql.connection.commit()
    cur.close()
    return redirect(url_for('masterbarang'))

@app.route('/view_dataproduk/<id_produk>', methods = ['POST', 'GET'])
def view_produk(id_produk):
    conn = mysql.connection.cursor()
    conn.execute("SELECT nama,harga,deskripsi,foto FROM data_poduk WHERE id_produk = %s", (id_produk,))
...
def formdatapenjualan():
    ...
    newid = cur.fetchall()
    cur.close()
    return render_template('formdatapenjualan.html', menu='penjualan',submenu='formpenjualan', data=newid)
...
def datapenjualan():
    cur = mysql.connection.cursor()
    cur.execute("SELECT id, nomor_invoice, nama_produk, sum(jumlah_produk_dibeli*harga_jual) as total, nama_pembeli, no_resi_kode_booking FROM data_penjualan group by id")
...
#clustering
    ...
    clustered_data['Nor_Recency'] = clustered_data['Nor_Recency'].round(3)
    clustered_data['Nor_Frequency'] = clustered_data['Nor_Frequency'].round(3)
    clustered_data['Nor_Monetary'] = clustered_data['Nor_Monetary']
...
#hasil normalisasi
    ...
    # This is the same as: transposed = np.array(list_of_lists).T.tolist()
    #hasil 
    return render_template('kmeans.html', menu='clustering',submenu='kmeans', label=label, data=clustered_data)
